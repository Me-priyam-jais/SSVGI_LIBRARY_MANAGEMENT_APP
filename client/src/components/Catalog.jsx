...
const Catalog = () => {
  ...
  const openReturnBookPopup = (bookId, email) => {
    ...
    console.log(email);
    setBorrowedBookId(bookId.book._id);
    setEmail(email);
    ...
    <>
      <main className="relative flex-1 p-6 pt-28">
        ...
        <header className="flex flex-col gap-3 sm:flex-row md:items-center">
          <button
            className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rouded-tl-lg sm:rounded-bl-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${
              filter === "borrowed"
          ...
          <button
            className={`relative rounded sm:rounded-tl-none sm:rouded-bl-none sm:rounded-tr-lg sm:rounded-br-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${
              filter === "overdue"
                ? "bg-black text-white border-black "
                : " bg-gray-200 text-black border-gray-200 hover-bg-gray-300"
            }`}
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        {book.returnDate ? (
                          ...
                          <PiKeyReturnBold
                            onClick={() => {
                              openReturnBookPopup(book, book?.user.email);
                            }}
                            className="w-6 h-6"
                          />
                        )}
