...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (registerationAttemptByUser.length >= 5) {
      return next(
        new ErrorHandler(
          "You have exceeded  the number of registeration attmepts. Please contact support.",
          400
        )
      );
    }
    if (password.length < 8 || password.length > 16) {
      return next(
        new ErrorHandler(
          "The Password must contain characters betweeen 8 and 16"
        )
      );
    }
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "SSVGI Librabry Management System Password Recovery",
      message,
    });
    res.status(200).json({
      success: true,
...
export const updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorHandler(
        "All passwords must contain between 8 and 16 characters.",
        400
      )
    );
  }
