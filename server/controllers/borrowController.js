...
export const recordBorrowedBook = catchAsyncErrors(async (req, res, next) => {
  ...
  if (isAlreadyBorrowed) {
    return next(new ErrorHandler("Book is being already borrowed", 400));
  }
  book.quantity -= 1;
...
export const returnBorrowedBook = catchAsyncErrors(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    message:
      fine !== 0
        ? `The book has been returned successfully. The total chagres, including a fine,are ₹ ${
            book.price + fine
          }`
        : `The book has been returned successfully. The total chagres are ₹ ${book.price}`,
  });
});
...
export const borrowedBooks = catchAsyncErrors(async (req, res, next) => {
  ...
  const borrows = await Borrow.find({ "user.id": userId }).populate(
    "book",
    "title price"
  );
  res.status(200).json({
